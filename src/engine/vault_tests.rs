...
fn test_save_text_append() {
    ...
    let contents = fs::read_to_string(&append_file).unwrap();
...
fn test_save_text_append_separator() {
    ...
    let append_file = vault.join("Notes.md");
    fs::write(&append_file, "Existing content").unwrap();
...
fn test_save_image() {
    ...
    let content = Content::Image { data: image_data, ext: "png" };
...
fn test_save_image_empty() {
    ...
    let cfg = test_config(&vault);
    let content = Content::Image { data: vec![], ext: "png" };

...
На Linux `make install` также ставит иконку в `~/.local/share/icons/hicolor/...` и копирует `assets/scoria.desktop` в `~/.local/share/applications/`, чтобы в лаунчере (KDE, GNOME и т.д.) у приложения была иконка.
...
Ветка Linux в `install.sh` скачивает бинарник из релиза, создаёт `~/.local/share/applications/scoria.desktop` и ставит иконки в `~/.local/share/icons/hicolor/` (PNG и/или SVG). В `.desktop` по возможности задаётся **абсолютный путь в `Icon=`**, чтобы в KDE Application Launcher и аналогах иконка находилась даже если тематическое имя `scoria` в системе не установлено.
...
Мастер спрашивает папку установки, затем отдельный шаг с чекбоксом ярлыка на **рабочем столе**, затем сводку. После установки на финальном экране может быть пункт **«Launch Scoria now»**. Ярлыки в **меню «Пуск»** и опционально на **рабочем столе** создаются в стандартных пользовательских папках оболочки (корректно и при установке на диск не `C:`). Отдельного экрана с лицензией нет.
...
```powershell
powershell -File scripts/windows/gen-winget-manifests.ps1 -Version 0.2.5 -Sha256 "<sha256 scoria-windows-x86_64.zip>"
```
...
Библиотеки для сборки и запуска (GTK нужен для **настроек**; трей использует **StatusNotifierItem** по D-Bus, без `libappindicator`):
...
> **GNOME**: чтобы значок появился в верхней панели, обычно нужно расширение [AppIndicator](https://extensions.gnome.org/extension/615/appindicator-support/) (`gnome-shell-extension-appindicator`). Без него `scoria save` и `scoria settings-gui` работают; может не отображаться только интеграция с треем.

> **Иконка в трее (Linux)**: используется протокол StatusNotifierItem с встроенной **pixmap**-иконкой, чтобы KDE, Hyprland и другие SNI-хосты стабильно показывали значок (в т.ч. когда по имени темы иконка не находится).
...
На Linux настройки можно открыть из терминала (в том числе когда трей не виден):

```bash
scoria settings-gui
```

На Windows `scoria settings-gui` открывает нативное окно; вёрстка и шрифт масштабируются с **DPI дислея** (например 150%). Запуск трея из меню «Пуск» или автозагрузки **не** открывает окно консоли.
...
| **Check for updates** / **Проверить обновления** | Во время проверки пункт меняется на **«Checking…»** / **«Проверка…»** и блокируется. Linux/macOS: скачать и установить при наличии обновления; Windows: уведомление и страница релизов для MSI/winget |
...
| `language` | *(пусто)* | Язык: пусто = авто **английский** или **русский** по ОС (`LANG` / `LC_*`, на Windows — локаль интерфейса); явно `en` или `ru` |

### Язык интерфейса

Доступны **английский** и **русский** (меню трея, уведомления, окно настроек).

- В **Настройках…** выберите **English** или **Русский**. Если в конфиге `language` пустой, диалог откроется с языком, который Scoria вывела из системы.
- В файле конфига: `language = ""` — автоопределение; `language = "en"` или `language = "ru"` — фиксированный язык.
...
В CI выполняются `cargo fmt --all --check`, `cargo clippy --all-targets -- -D warnings` и `cargo test --locked`.
